...
export default function HeroForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // This only opens WhatsApp with the details pre-filled — it is a
    // handoff, not a confirmed enquiry, so it does not fire the
    // 'form_submit'/'enquiry_submitted' conversion events.
    ...
    return (
      <div className="bg-green-500/20 backdrop-blur-sm border border-green-400/40 rounded-xl p-5 max-w-lg mx-auto mb-8 text-center">
        ...
        <p className="text-green-200 text-sm mt-1">
          Press Send in WhatsApp to message us — we typically reply within a few hours.
        </p>
      </div>
    );
  }
  ...
  return (
    <form
      ...
      // Fallback for the rare case JavaScript fails to load: instead of a
      // default GET submission (which would append these details to the
      // page URL), this posts to the real enquiry endpoint so the details
      // still reach the business instead of leaking into the address bar.
      method="post"
      action="/api/enquiry"
      className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-5 max-w-lg mx-auto mb-8"
    >
      <p id="hero-form-heading" className="text-white font-semibold text-sm mb-4 text-center">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          <label htmlFor="hero-suburb" className="sr-only">Your suburb</label>
          <input
            id="hero-suburb"
            type="text"
            name="suburb"
        ...
        {/* Hidden defaults so a no-JS fallback POST to /api/enquiry still
            satisfies the server's required fields. */}
        <input type="hidden" name="service" value="not-sure" />
        <input type="hidden" name="message" value="Sent from the homepage quick form (no project details provided)." />
        <button
          type="submit"
        ...
        <p className="text-white/70 text-xs text-center">
          Opens WhatsApp with your details pre-filled — press Send there to message us.
        </p>
      </div>
    </form>
  );
